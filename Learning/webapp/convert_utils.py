...
def get_wolrds(distance, stroke):
    """Given an event distance and stroke, return the list of the four
    wolrd records at the distance and stroke.
    """
...
def perform_conversions(times):
    """Given a list of swim times as strings, return the average time as a string,
        as well as the reversed list of times and list of scaled swim times as numbers (floats).
        """
    converts = []
    for t in times:
    ...
    scaled = [hfpy_utils.convert2range(
        n, 0, from_max, 0, 350) for n in converts]
    return average, times, scaled  # Returned as tuple
